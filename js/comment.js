...
;(function (root, factory) {
  ...
  JL.Renders = {
    box: {
      tpl: [
        '<section class="box" id="Will__commentBox">',
          '<div class="com-avatar"><img id="Will__loginAvatar" src="/img/unsigned_avatar.jpg" alt="avatar"></div>',
          '<div class="com-text">',
            '<div class="main">',
              '<textarea class="text-area-edited show" id="Will__editBox" placeholder="欢迎评论！"></textarea>',
              '<div class="text-area-preview" id="Will__previewBox"></div>',
            '</div>',
            '<div class="switch">',
              '<div class="switch-item on" id="Will__editSwitcher" onclick="Will.Actions.editPreviewSwitch(\'edit\')">编辑</div>',
              '<div class="switch-item" id="Will__previewSwitcher" onclick="Will.Actions.editPreviewSwitch(\'preview\')">预览</div>',
            '</div>',
            '<div class="button" onclick="Will.Actions.postComment()">提交</div>',
          '</div>',
        '</section>'
      ].join(''),
      update: function () {
        ...
        // 默认头像路径 /img/Will.jpg
        $('Will__loginAvatar').src = userInfo.avatar_url || '/img/unsigned_avatar.jpg';
      }
    },
    list: {
      tpl: [
        '<section class="list-wrap" id="Will__commentList">',
      ...
      update: function (page, comments, list, callback) {
        ...
        if (comments === 0) {
            ...
            item = [
              '<li class="item">',
                ...
                '<div class="user-comment">',
                  '<div class="user-comment-header" id="Will__comment_' + list[i].id + '_reactions">',
                    ...
                    '<span class="like" onclick="Will.Actions.like(' + list[i].id + ')">点赞</span>',
                    '<span class="like-num">' + list[i].reactions.heart + '</span>',
                    '<span class="reply" onclick="Will.Actions.reply(\'' + list[i].user.login + '\', \'' + (list[i].body_html || list[i].body).replace(/<[^>]+>|\s|[\r\n]/g, ' ') + '\')">回复</span>',
                  '</div>',
          ...
          if (allPages === 1) {
            ...
            for (var i = 1; i <= allPages; i++) {
              if (i === page) {
                ...
                pageItem = '<a href="javascript: Will.Actions.pageJump(' + i + ');" class="item">' + i + '</a>';
              }
              pageList.push(pageItem);
            }
            if (page !== 1) {
              pageList.unshift('<a href="javascript: Will.Actions.pageJump(' + (page - 1) + ');" class="item">上页</a>');
            }
            if (page !== allPages) {
              pageList.push('<a href="javascript: Will.Actions.pageJump(' + (page + 1) + ');" class="item">下页</a>');
            }
          } else if (allPages > perNavPageMaxSize) {
            if (page <= perNavPageMaxSize) {
              ...
              pageList.push('<a href="javascript: Will.Actions.pageJump(' + (page + 1) + ');" class="item">下页</a>');
              pageList.push('<a href="javascript: Will.Actions.pageJump(' + allPages + ');" class="item">末页</a>');
              ...
              pageList.push('<a href="javascript: Will.Actions.pageJump(1);" class="item">首页</a>');
              pageList.push('<a href="javascript: Will.Actions.pageJump(' + (page - 1) + ');" class="item">上页</a>');
          ...
          html = [
            '<header class="list-header">总共 <span class="comments-num" id="Will__commentsNum">' + JL.issueComments + '</span> 条评论</header>',
            '<ul class="list">',
              htmlList.join(''),
            '</ul>',
            '<div class="page-nav">',
        ...
        $('Will__commentList').innerHTML = html;
      ...
      reactionUpdate: function (commentId, reactions) {
        ...
        for (var i = 0, len = reactions.length; i < len; i++) {
          if (userId === reactions[i].user.id) {
            ...
            addClass($('Will__comment_' + commentId + '_reactions').getElementsByClassName('like')[0], 'liked');
            $('Will__comment_' + commentId + '_reactions').getElementsByClassName('like')[0].innerHTML = '已赞';
            break;
          }
        }
      },
      addOne: function (data) {
        ...
        var item = [
          ...
          '</div>',
          '<div class="user-comment">',
            '<div class="user-comment-header" id="Will__comment_' + data.id + '_reactions">',
              '<span class="post-name">' + data.user.login +  '</span>',
              '<span class="post-time">' + formatDate('yyyy-MM-dd hh:mm', new Date(data.created_at)) + '</span>',
              '<span class="like" onclick="Will.Actions.like(' + data.reactions.heart + ')">点赞</span>',
              '<span class="like-num">' + data.reactions.heart + '</span>',
              '<span class="reply" onclick="Will.Actions.reply(\'' + data.user.login + '\', \'' + (data.body_html || data.body).replace(/<[^>]+>|\s|[\r\n]/g, ' ') + '\')">回复</span>',
            '</div>',
            '<div class="user-comment-body">' + (data.body_html || data.body) + '</div>',
          '</div>'
        ].join('');
        oLi.innerHTML = item;
        var oUl = $('Will__commentList').getElementsByTagName('ul')[0];
        if (oUl) {
          oUl.insertBefore(oLi, oUl.firstChild);
          $('Will__commentsNum').innerHTML = JL.issueComments + 1;
        } else {
          $('Will__commentList').innerHTML = [
            '<header class="list-header">总共 <span class="comments-num" id="Will__commentsNum">' + (JL.issueComments + 1) + '</span> 条评论</header>',
            '<ul class="list">',
              '<li class="item">',
                item,
              '</li>',
    ...
    signBar: {
      tpl: [
        '<div class="sign-bar" id="Will__commentSignBar">',
        '</div>'
      ].join(''),
      update: function () {
        ...
        if (token && userInfo) {
          ...
          html = [
            ...
            '<span class="sign-link" onclick="Will.Actions.signOut()">退出</span>'
          ].join('');
        } else {
          html = [
        ...
        $('Will__commentSignBar').innerHTML = html;
      }
    },
    tips: {
      tpl: '<section class="tips clearfix" id="Will__comment_tips">注：评论支持 markdown 语法！</section>',
      update: function () {
        var userInfo = localStorage.getItem(constants.USER_INFO_KEY);
        var handler = '';
        // 如果文章还没关联 issue 并且登录账号是自己时
        if (userInfo && JSON.parse(userInfo).login === JL.options.owner && JL.issueNumber === 0) {
          handler = '<a href="javascript: Will.Actions.createIssue();" class="init" title="文章关联 issue">初始化评论</a>';
        }
        $('Will__comment_tips').innerHTML = handler + '注：评论支持 markdown 语法！';
      }
    },
    ...
    loading: {
      ...
      remove: function () {
        var oLoading = $('Will__loadingMask');
  ...
  JL.Actions = {
    ...
    editPreviewSwitch: function (flag) {
      if (flag === 'edit') {
        removeClass('Will__previewSwitcher', 'on');
        addClass('Will__editSwitcher', 'on');
        removeClass('Will__previewBox', 'show');
        addClass('Will__editBox', 'show');
      } else {
        removeClass('Will__editSwitcher', 'on');
        addClass('Will__previewSwitcher', 'on');
        removeClass('Will__editBox', 'show');
        addClass('Will__previewBox', 'show');
        var text = $('Will__editBox').value.trim();
        // 安全转义
        ...
        if (text) {
          JL.Requests.markdown({
            ...
            $('Will__previewBox').innerHTML = res;
          });
        } else {
          $('Will__previewBox').innerHTML = '';
        }
      }
    },
    postComment: function () {
      ...
      var body = $('Will__editBox').value.trim();
      // 安全转义
    ...
    like: function (commentId) {
      var oLiked = $('Will__comment_' + commentId + '_reactions').getElementsByClassName('liked');
      var oLike = $('Will__comment_' + commentId + '_reactions').getElementsByClassName('like')[0];
      var oNum = $('Will__comment_' + commentId + '_reactions').getElementsByClassName('like-num')[0];
      var accessToken = localStorage.getItem(constants.ACCESS_TOKEN_KEY);
      var userInfo = localStorage.getItem(constants.USER_INFO_KEY);
      if (oLiked.length) {
        return false;
